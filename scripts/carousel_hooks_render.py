...
def render():
    ...
    S.append(f"""<article class="slide">
      ...
      <div class="viz chips">
        {chip('«Сегодня хочу поговорить о питании…»','слабо')}
        {chip('«Ты ешь мало, а вес стоит. Дело не в еде.»','сильно',on=True)}
    ...
    S.append(f"""<article class="slide">
      ...
      <div class="viz dvs">
        ...
        <div class="b good"><div class="l">Конфликт</div><div class="t">«Ты делаешь всё правильно — и не растёшь»</div></div>
      </div>
    </article>""")
    ...
    S.append(f"""<article class="slide">
      ...
      <div class="mid">
        ...
        <div class="src">
          {hc('Ошибка','«Ты греешь сковороду не так»')}
          {hc('Результат','«Собрал ролик за вечер — без съёмок»')}
          {hc('Конфликт','«Дорогая кухня, дешёвый контент»')}
          {hc('Вопрос новичка','«Почему у меня выходит дёшево?»')}
          {hc('До / после','«Было пусто. Стало — очередь»')}
          {hc('Разрушение мифа','«Талант тут ни при чём»')}
        </div>
      </div>
    </article>""")
    ...
    S.append(f"""<article class="slide slide--plain">
      ...
      <div class="mid">
        ...
        <div class="dvs" style="margin-top:14px">
          <div class="b bad"><div class="l">Слабо</div><div class="t">«Расскажу про пользу режима дня»</div></div>
          <div class="b good"><div class="l">Сильно</div><div class="t">«Ты не ленивый. Ты просто не спишь»</div></div>
        </div>
      </div>
