...
SCENE3 = f"""<section class="story">
  <div class="sparks">{sparks()}</div>
  <div class="eb">AlovLab · курс с нуля</div>
  <h2>Путь новичка</h2>
  <div class="sub">открыл → повторил → получил результат</div>
  <div class="steps">
    <div class="step"><div class="vis">{LAPTOP}</div><div class="tx"><b>Чистый лист</b><span>пустой чат, ни одного запроса</span><span class="tag">0 опыта</span></div></div>
    <div class="arrowdown">↓</div>
    <div class="step"><div class="vis" style="width:132px">{PROMPT_BUBBLE}</div><div class="tx"><b>Первый запрос</b><span>простыми словами, без терминов</span><span class="tag">по шаблону</span></div></div>
    <div class="arrowdown">↓</div>
    <div class="step res"><div class="vis">{RESMINI}</div><div class="tx"><b>Первый результат</b><span>текст · картинка · видео</span><span class="tag">готово</span></div></div>
  </div>
...
# ---------------- СЦЕНА 4 · тарифы ----------------
def tc(nm, sub, pr, inside, pro=False, hit=False):
    ics = "".join(f'<span class="ic">{i}</span>' for i in inside)
    hitb = '<span class="hit">ХИТ</span>' if hit else ''
    return (f'<div class="tc{" pro" if pro else ""}">{hitb}'
            f'<div class="hd"><div class="nm">{nm}<small>{sub}</small></div><div class="pr">{pr}</div></div>'
            f'<div class="inside">{ics}</div></div>')

SCENE4 = f"""<section class="story">
  <div class="sparks">{sparks()}</div>
  <div class="eb">AlovLab · три уровня входа</div>
  <h2>Заходишь <b>с любого</b></h2>
  <div class="sub">выбираешь глубину — система одна</div>
  <div class="tcards">
    {tc("МИНИ", "попробовать", "2 999 ₽", ["Модуль «Земля Слов»", "закрытый ТГ-канал", "ИИ-ассистент"])}
    {tc("БАЗОВЫЙ", "весь курс", "14 990 ₽", ["6 уроков · 6 Земель", "канал + ассистент", "обновления"])}
    {tc("ПРО", "курс + менторство", "49 990 ₽", ["всё из Базового", "доступ навсегда", "сертификат", "проверка заданий", "менторство", "клуб"], pro=True, hit=True)}
  </div>
  <div class="tfoot">Гарантия возврата <b>14 дней</b></div>
  {foot("без таймеров и «мест»")}
</section>"""

for name, html in [("scene3-path", SCENE3), ("scene4-tariffs", SCENE4)]:
