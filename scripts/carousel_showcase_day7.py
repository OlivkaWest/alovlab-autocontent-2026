...
def cover(hw, ho, sub, ic, pg):
  ...
  <div class="top"><span class="eb">AlovLab · система вместо хаоса</span><span class="pg">{pg}<b> / 8</b></span></div>
...
def roles_slide():
  ...
  <div class="sparks">{sparks()}</div>
  <div class="top"><span class="eb">Решение · 7 ролей</span><span class="pg">5<b> / 8</b></span></div>
...
def example_slide():
    ...
    return f"""<article class="slide">{DEFS}
  <div class="sparks">{sparks()}</div>
  <div class="top"><span class="eb">Готовый пример</span><span class="pg">6<b> / 8</b></span></div>
...
SLIDES = [
 cover("Сто постов", "не спасут.", "Слабую идею не вытянет объём. Вытянет связка: одна идея на всю неделю.", "link", 1),
 blk("Проблема","Один пост","выстрелил.","А дальше — снова с нуля. Один раз повезло,","повторить не смог: не знаешь, почему сработало.","redo", 2),
 blk("Причина","Удача не","масштабируется.","Отдельные хиты не складываются в результат.","Повторяемость даёт не талант, а система.","sys", 3),
 blk("Ошибка","Ты ищешь","семь тем.","А нужна одна. Каждый день новая тема — это хаос.","Сила не в количестве, а в глубине одной идеи.","fork", 4),
 roles_slide(),
 example_slide(),
 prompt_slide(),
 cta("Собери","связку.",
